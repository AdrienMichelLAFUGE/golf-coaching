-- Fix parent invitation acceptance runtime failure on staging:
-- - avoid dependency on gen_random_bytes() during invitation acceptance checks
-- - harden hash helper when salt is null/empty
...
begin
  if effective_salt is null then
    -- 32 hex chars without relying on gen_random_bytes()
