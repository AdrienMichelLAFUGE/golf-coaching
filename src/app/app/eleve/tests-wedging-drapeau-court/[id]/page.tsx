...
import {
  ...
  getWedgingDrapeauCourtResultOptions,
...
type AttemptValue = WedgingDrapeauCourtResultValue | null;

const createEmptyAttempts = () =>
  Array(WEDGING_DRAPEAU_COURT_TEST.attemptsPerSubtest).fill(null) as AttemptValue[];
...
export default function StudentWedgingDrapeauCourtPage() {
  ...
  useEffect(() => {
    const loadAssignment = async () => {
      ...
      if (assignmentData.test_slug !== WEDGING_DRAPEAU_COURT_SLUG) {
  ...
  const objectivation = useMemo(
    () => computeWedgingDrapeauCourtObjectivation(indexLabel, totalScore),
    [indexLabel, totalScore]
  );

  const handleResultChange = (index: number, value: string) => {
    if (!isWedgingDrapeauCourtResultValue(value)) return;
    setAttempts((prev) => {
      const next = [...prev];
      next[index] = value as WedgingDrapeauCourtResultValue;
      return next;
    });
  };
        ...
        attempts: attempts
          .map((value, index) =>
            value
              ? {
                  index: index + 1,
                  situation: WEDGING_DRAPEAU_COURT_SEQUENCE[index],
                  result: value,
                }
              : null
          )
          .filter(
            (
              ...
              index: number;
              situation: WedgingDrapeauCourtSituation;
              result: WedgingDrapeauCourtResultValue;
            } => Boolean(value)
          ),
      },
    ],
      ...
      {loading ? (
        ...
        <div className="space-y-6">
          <PageHeader
            ...
            title={WEDGING_DRAPEAU_COURT_TEST.title}
            subtitle={
              <>
                <p>{WEDGING_DRAPEAU_COURT_TEST.description}</p>
                <p>
          ...
          <section className="panel rounded-2xl p-6">
            ...
            <div className="mt-4 grid gap-3 md:grid-cols-3">
              {getWedgingDrapeauCourtResultOptions().map((option) => (
                <div
                  key={option.value}
      ...
      <TestResultModal
        ...
        items={[
          { label: "Total points", value: totalScore.toString() },
          {
            ...
            value: getWedgingDrapeauCourtEquivalentIndexLabel(totalScore),
          },
        ]}
      />
    </RoleGuard>
